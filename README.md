...
6. Test a commit and push it to make sure everything is connected with ```git commit -m "Go Topaz!"```.
...
Did you read all of the directions before starting? If so write `git commit -m "Go Topaz!"` instead of `git commit -m "first commit"` for your first commit.
